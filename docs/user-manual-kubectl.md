...
``` openssl req -x509 -new -nodes -key ca.key -sub "/CN=Webhook Issuer" -days 10000 -out ca.crt ```
...
8. Add the certificate authority as bas64 encoded CA data (`base64 ./ca.crt`) to the ValidatingWebhookConfiguration under the webhooks[].clientConfig.caBundle field. It should look something like:
```yaml

...
func TestDaemonSetForScannerSetMergesEnvWithDeterministicPrecedence(t *testing.T) {
	m := runtimeCacheAuditConfig()
	m.Spec.Containers.RuntimeCache.Env = []corev1.EnvVar{
		{Name: "SHARED", Value: "cache"},
		{Name: "MONDOO_AUTO_UPDATE", Value: "cache-override"},
		{Name: "SHARED", Value: "cache-last"},
		{Name: "TOP_ONLY", Value: "true"},
	}
	set := v1alpha2.NodeScannerSet{
		Name: "workers",
		Env: []corev1.EnvVar{
			{Name: "SHARED", Value: "set"},
			{Name: "SET_ONLY", Value: "true"},
			{Name: "SHARED", Value: "set-last"},
		},
	}

	ds, err := DaemonSetForScannerSet("cnspec:test", constants.BusyBoxImage, "", testClusterUID, m, v1alpha2.MondooOperatorConfig{}, set)
	require.NoError(t, err)

	env := ds.Spec.Template.Spec.Containers[0].Env
	uniqueNames := map[string]struct{}{}
	for _, item := range env {
		uniqueNames[item.Name] = struct{}{}
	}
	assert.Len(t, uniqueNames, len(env), "environment variable names must be unique")

	envMap := envToMap(env)
	assert.Equal(t, "set-last", envMap["SHARED"])
	assert.Equal(t, "cache-override", envMap["MONDOO_AUTO_UPDATE"])
	assert.Equal(t, "true", envMap["TOP_ONLY"])
	assert.Equal(t, "true", envMap["SET_ONLY"])
}

func TestDaemonSetNameForScannerSetUsesDaemonSetLengthBudget(t *testing.T) {
	prefix := strings.Repeat("a", 45)
	name := DaemonSetNameForScannerSet(prefix, "control-plane-workers")
	assert.LessOrEqual(t, len(name), k8s.DaemonSetNameChildResourceSafeMaxLength)
	assert.Greater(t, len(name), k8s.ResourceNameMaxLength)
}

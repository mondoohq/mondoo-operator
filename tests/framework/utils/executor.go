...
func (*CommandExecutor) ExecuteCommandWithTimeout(timeout time.Duration, command string, arg ...string) (string, error) {
	...
	for {
		...
		case <-time.After(timeout):
			if interruptSent {
				zap.S().Infof("timeout waiting for process %s to return after interrupt signal was sent. Sending kill signal to the process", command)
				var e error
				if err := cmd.Process.Kill(); err != nil {
					zap.S().Errorf("Failed to kill process %s: %v", command, err)
			...
			zap.S().Infof("timeout waiting for process %s to return. Sending interrupt signal to the process", command)
			if err := cmd.Process.Signal(os.Interrupt); err != nil {
				zap.S().Errorf("Failed to send interrupt signal to process %s: %v", command, err)
...
func startCommand(env []string, command string, arg ...string) (*exec.Cmd, io.ReadCloser, io.ReadCloser, error) {
	...
	if err != nil {
		zap.S().Warnf("failed to open stdout pipe: %+v", err)
	}
	stderr, err := cmd.StderrPipe()
	if err != nil {
		zap.S().Warnf("failed to open stderr pipe: %+v", err)
	}
...
func logFromReader(reader io.ReadCloser) {
	...
	for in.Scan() {
		lastLine = in.Text()
		zap.S().Debug(lastLine)
	}
}

func logOutput(stdout, stderr io.ReadCloser) {
	if stdout == nil || stderr == nil {
		zap.S().Warnf("failed to collect stdout and stderr")
		return
	}
...
func logCommand(command string, arg ...string) {
	zap.S().Debugf("Running command: %s %s", command, strings.Join(arg, " "))
}

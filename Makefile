...
test/integration: manifests generate generate-manifests load-minikube
	...
	go test -ldflags $(LDFLAGS) -v -timeout 30m -p 1 ./tests/integration/...
...
test/integration/ci: manifests generate generate-manifests gotestsum load-minikube
	...
	$(GOTESTSUM) --junitfile integration-tests.xml -- ./tests/integration/... -ldflags $(LDFLAGS) -v -timeout 30m -p 1

...
type ConsoleIntegration struct {
	...
	// AutoCreate lets the operator create (or re-attach to) the Mondoo console integration
	// itself when the Secret referenced by .spec.mondooTokenSecretRef holds a credential that
	// is not already bound to an integration: a space- or org-scoped registration token or
	// service account with permission to manage integrations (e.g. the deployment-manager
	// role). It has no effect when the token already carries an integration MRN or when the
	// Secret referenced by .spec.mondooCredsSecretRef already exists.
	// +kubebuilder:default=true
	// +optional
	AutoCreate *bool `json:"autoCreate,omitempty"`

	// Name is the display name for an integration created by the operator. If empty, it
	// defaults to "mondoo-operator-" followed by the first 8 characters of the cluster UID.
	// +optional
	Name string `json:"name,omitempty"`

	// DeletionPolicy controls what happens to an operator-created integration when this
	// MondooAuditConfig is deleted. "Delete" removes the integration from the Mondoo console
	// (assets and their history are kept); "Retain" leaves it in place. Integrations that were
	// not created by the operator are never deleted, regardless of this setting.
	// +kubebuilder:validation:Enum=Delete;Retain
	// +kubebuilder:default=Delete
	// +optional
	DeletionPolicy IntegrationDeletionPolicy `json:"deletionPolicy,omitempty"`
}

// IntegrationDeletionPolicy describes how to handle an operator-created console integration
// when the owning MondooAuditConfig is deleted.
type IntegrationDeletionPolicy string

const (
	IntegrationDeletionPolicyDelete IntegrationDeletionPolicy = "Delete"
	IntegrationDeletionPolicyRetain IntegrationDeletionPolicy = "Retain"
)

// AutoCreateIntegration returns whether the operator may create the console integration
// itself. Defaults to true when the field is unset.
func (c ConsoleIntegration) AutoCreateIntegration() bool {
	return c.AutoCreate == nil || *c.AutoCreate
}

// ConsoleIntegrationActive returns whether console integration features (CheckIn, status
// reporting, pause/resume) should be active. This is true when either the user explicitly
// set enable: true, or the autoCreate flow successfully created an integration (tracked
// via status.IntegrationMRN to avoid spec mutations that conflict with GitOps orchestrators).
func (m *MondooAuditConfig) ConsoleIntegrationActive() bool {
	return m.Spec.ConsoleIntegration.Enable || m.Status.IntegrationMRN != ""
}

// JobOverrides defines customizations for the scan Jobs generated by the operator. Use this
// to control scheduling and cleanup of scan Jobs directly instead of relying on mutating
// webhooks (e.g. Kyverno policies patching the generated CronJobs).
...
type KubernetesResources struct {
	...
	// JobOverrides customizes the Jobs generated for Kubernetes resource scanning,
	// including the CronJobs for external clusters.
	// +optional
	JobOverrides JobOverrides `json:"jobOverrides,omitempty"`
}
...
type Nodes struct {
	...
	// variable with the same name, the value specified here will override it.
	Env []corev1.EnvVar `json:"env,omitempty"`
	// JobOverrides customizes the Jobs generated for node scanning. Only applicable for CronJob
	// style. NodeSelector is ignored because node scan pods are pinned to a specific node.
	// +optional
	JobOverrides JobOverrides `json:"jobOverrides,omitempty"`
}
...
type Containers struct {
	...
	// JobOverrides customizes the Jobs generated for container image scanning.
	// +optional
	JobOverrides JobOverrides `json:"jobOverrides,omitempty"`
}
...
type MondooAuditConfigStatus struct {
	...
	// IntegrationMRN is the MRN of the console integration created by the autoCreate flow.
	// Stored in status (not spec) so the operator can track auto-created integrations without
